...
use serde::{Deserialize, Serialize}; //by looking at .toml, serde_json is enabled
use std::fs;
...
#[derive(Serialize)]
struct OutputToJS {
    // Tauri official guide really has the worst beginner documentation
...
async fn open_splashscreen(window: tauri::Window) {
    ...
    fs::write(
        "/Users/sbritorodr/Desktop/PROYECTOS/allsum/test.txt",
        format!("{:?}", allwindows),
    )
    .expect("Unable to write file");
    window
        .get_window("splashscreen")
        .expect("Cannot get window name 'splashscreen'")
        .show()
        .expect("Cannot show window name 'splahscreen'")
}

#[tauri::command(async)]
fn text_hash_processing(inputStr: &str, hashType: &str, isFileModeOn: bool) -> String {
    let input_bytes: Vec<u8> = if isFileModeOn {
...
fn main() {
    ...
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![
            ...
            close_splashscreen,
            open_splashscreen,
            algorithms_selector_string
        ]) //add all used commands here to communicate to js
